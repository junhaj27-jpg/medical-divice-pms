...
class WebApiTests(Base):
    ...
    def test_integrated_assistant_drawer_keeps_core_work_links(self):
        self.client.login(username="raqa",password="test-password-123"); response=self.client.get(reverse("dashboard")); self.assertContains(response,"RA 업무 센터"); self.assertContains(response,reverse("assistant-chat")); self.assertContains(response,reverse("complaint-create")); self.assertContains(response,reverse("capa-list")); self.assertContains(response,reverse("recall-list")); self.assertContains(response,reverse("report-list"))
